...
module.exports = function Master(options) {
    ...
    this.connect = function(callback) {
        var vfs = createVfs(options);
        var client = vfs._client;
        delete vfs._client;

        client.emitter.once("connect", function onConnect() {
            client.emitter.off("disconnect", onDisconnect);
            callback(null, vfs);
        });
        client.emitter.once("disconnect", function onDisconnect(e) {
            client.emitter.off("connect", onConnect);
...
// fsOptions - object containing port, host, user and pass. All
// the properties are strings except for port, which is a number.
function createVfs(fsOptions) {
    var ftpClient = new FTP(fsOptions);
